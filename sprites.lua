...
function load_sprites(filename)
    ...
    for step = 0,self.animation_steps-1 do
        ...
        for dir = 1,self.dir_steps do
            dirs[dir] = love.graphics.newQuad(step*self.h, remap_dir[dir]*self.h, self.h, self.h, self.image)
        end
    end

    function self.draw(self, x, y, anim_frame, angle)
        ...
        love.graphics.draw(self.image, self.quads[anim_frame][dir], x, y)
    end
